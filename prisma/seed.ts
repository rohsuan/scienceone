...
<p>The canonical ensemble describes a system in thermal contact with a heat bath at temperature T. The partition function Z = Σ exp(-βE_i) encodes all thermodynamic information about the system.</p>`,
  ...
  // ---- Simulations ----
  // Fetch the Physics subject for linking
  const physicsSubject = await prisma.subject.findUnique({
    where: { slug: "physics" },
  });

  if (!physicsSubject) {
    throw new Error("Physics subject not found — ensure subjects are seeded before simulations");
  }
  ...
  for (const sim of simulationsData) {
    ...
    console.log(`Simulation upserted: ${sim.title}`);
  }
